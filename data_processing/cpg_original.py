    ...
    for item in ast_edges:
        ...
        for y in item['end']:
            ...
            if combine(x_node, y_node) == True:   
                ...
                flag = False
                for sublist in duplist:  # check if the head node can be merged forward
        ...
        for item in same_var:
            for y in item:
                ...
                if x_node['code'].strip() == y_node['code'].strip() and x_node['type'].strip() in ['Argument','Identifier'] and y_node['type'].strip() in ['Argument','Identifier']:
                    item.append(x_node['key'].strip())  #当前的node可以和之前的合并
                    flag = True
                    break
            if flag:
                break
...
def main():
    ...
    parser.add_argument('--wv', default='devign_dataset/devign_wv_models/devign_train_subtoken_data')  # word2vc 
    parser.add_argument('--output_dir', default='devign_dataset/devign_cpg_c0')
