...
def graphGeneration(nodes, edges, edge_type_map, ver_edge_type_map):
    ...
    same_var = get_same_var(all_ast_edges, nodes)
    step2_ast_edges = var_combine(all_ast_edges, same_var)
    ...
    #add nsc
    nsc_edges_ = get_ncs_edges(all_ast_edges, 'NSC', nodes)
    nsc_edges = get_combine_ncs_edges(all_ast_edges, 'NSC', nodes, same_var)
...
def visual():
    ...
    file_name = "FFmpeg11_1.c"
    file_path = "dataset/devign_parsed"
    nodes_path = os.path.join(file_path, file_name, "nodes.csv")
    edges_path = os.path.join(file_path, file_name, "edges.csv")
    ...
    if index_map != None and graph != None: 
        visual_graph(nodes, index_map, graph, edge_type_ast, ver_edge_type_reduced, "graph/step2_reduced_"+file_name)
        #visual_graph_full(nodes_ids_to_nodes, edges, allowed_edge_types_reduced, "graph_reduced")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--csv', help='normalized csv files to process', default='fan_dataset/fan_parsed/')  #nodes  edges
    parser.add_argument('--src', help='source c files to process', default='fan_dataset/fan_raw_code/')
    parser.add_argument('--json_files', help = 'train and test and valid', default=['fan_dataset/fan_data_split/train_raw_code.json', 'fan_dataset/fan_data_split/test_raw_code.json', 'fan_dataset/fan_data_split/valid_raw_code.json'])
    parser.add_argument('--wv', default='fan_dataset/fan_wv_models/fan_train_subtoken_data')  # word2vec
    parser.add_argument('--output_dir', default='fan_dataset/fan_cpg_c1_2_')
    ...
    print("*"*100)
    train_output_path = open(os.path.join(output_dir, 'fan-train-v1.json'), 'w')
    test_output_path = open(os.path.join(output_dir, 'fan-test-v1.json'), 'w')
    valid_output_path = open(os.path.join(output_dir, 'fan-valid-v1.json'), 'w')
    output_files = [train_output_path, test_output_path, valid_output_path]

    train_num_path = open(os.path.join(output_dir, 'fan-train-num-v1.json'), 'w')
    test_num_path = open(os.path.join(output_dir, 'fan-test-num-v1.json'), 'w')
    valid_num_path = open(os.path.join(output_dir, 'fan-valid-num-v1.json'), 'w')
    num_files = [train_num_path, test_num_path, valid_num_path]

    train_file = open(os.path.join(output_dir, 'fan-train-file.json'), 'w')
    test_file = open(os.path.join(output_dir, 'fan-test-file.json'), 'w')
    valid_file = open(os.path.join(output_dir, 'fan-valid-file.json'), 'w')
